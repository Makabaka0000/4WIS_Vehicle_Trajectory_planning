    ...
    for j=1:4
        ...
        %     [x,y,theta]=SearchAStarPath([params_.task.x0,params_.task.y0,params_.task.theta0],[params_.task.xf,params_.task.yf,params_.task.thetaf]);
            ...
            if(ready_flag)
                ...
                save(strcat('E:\MATLAB 2019a\bin\ourscompara\ours\hard\res',num2str(ii*100+j*10+k)),'params_');
                DrawParkingScenariowoArrow();
                DrawTrajFootprints(params_.ha_result.x,params_.ha_result.y,params_.ha_result.theta);
                SaveFigure(['E:\MATLAB 2019a\bin\ourscompara\ours\hard\','hard_',num2str(ii*1000+j*100+k*10),'.png']);
                DrawParkingScenario();
                plot(params_.ha_result.x,params_.ha_result.y,'LineWidth',2,'Color',[0.8039,0.5216,0.2471]);
                SaveFigure(['E:\MATLAB 2019a\bin\ourscompara\ours\hard\','hard_',num2str(ii*1000+j*100+k*10+1),'.png']);
            else
                save(strcat('E:\MATLAB 2019a\bin\ourscompara\ours\hard\ig',num2str(ii*100+j*10+k)),'params_');
            end
        end
    end
end
