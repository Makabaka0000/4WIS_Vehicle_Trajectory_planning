function[x,y,theta,pattern]=SearchForwardHybridAstar(start_config,end_config)
...
%         hold on
    ...
    for i=size(task_point,1):-1:1
        ...
        for k=1:point_num
           posex=[posex,child_node.x+(k+1)*(-child_node.x+task_point(i,1))/(point_num+1)];
           posey=[posey,child_node.y+(k+1)*(-child_node.y+task_point(i,2))/(point_num+1)];
