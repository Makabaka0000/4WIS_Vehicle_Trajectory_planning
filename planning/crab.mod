...
minimize obj: 
tf + 
w_penalty * (sum{i in {2..Nfe}}((x[i] - x[i-1] - hi * v[i] * cos(theta[i]+phy[i]))^2 + (y[i] - y[i-1] - hi * v[i] * sin(theta[i]+phy[i]))^2 + (v[i] - v[i-1] - hi * a[i])^2 + 
(theta[i] - theta[i-1])^2 + (phy[i] - phy[i-1])^2 + (xf[i] - x[i] - Lfc * cos(theta[i]))^2 + (yf[i] - y[i] - Lfc * sin(theta[i]))^2 + (xr[i] - x[i] + Lrc * cos(theta[i]))^2 + (yr[i] - y[i] + Lrc * sin(theta[i]))^2));
