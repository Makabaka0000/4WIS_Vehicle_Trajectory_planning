function[x,y,theta,pattern]=SearchGSPBackwardHybridAstar(start_config,end_config,order)
    ...
    while((~isempty(openlist_))&&(ha_iter<=params_.hybrid_astar.max_search_iter)&&(~is_ha_search_complete))
        ...
        for ii=1:(params_.hybrid_astar.num_sampled_phy_in_expansion)
            ...
            child_g=cur_node.g+params_.hybrid_astar.simulation_step+params_.hybrid_astar.penalty_for_steering_change*abs(child_node.via_phy-cur_node.via_phy)+penalty_for_model_change;
        
            if(child_node.via_v>0)
                child_g=child_g+100*params_.hybrid_astar.penalty_for_backward;
            end
            ...
            if(counter_to_trigger_rs_curve>params_.hybrid_astar.threshold_to_trigger_rs)
                counter_to_trigger_rs_curve=0;
%                 rs_path_seg=GenerateRsPath(child_node,end_config);
%                 rs_path_seg=GenerateDuBinsPath(child_node_crab,end_config);
                if order==2
                    connect_path=GenerateConnectPath(child_node);                    
                elseif order==1
                    connect_path=GenerateConnectPatho1(child_node,end_config);
                end
       ...
       %拓展斜向运动的节点
        for ii = 1 : size(expansion_crab,1)
            ...
            child_crab_g = cur_node.g + params_.hybrid_astar.simulation_step+penalty_for_model_change + params_.hybrid_astar.penalty_for_steering_change*abs(child_node_crab.via_phy-cur_node.via_phy);
            if(child_node_crab.via_v>0)
                child_crab_g=child_crab_g+100*params_.hybrid_astar.penalty_for_backward;
            end            
            ...
            if(counter_to_trigger_rs_curve>params_.hybrid_astar.threshold_to_trigger_rs)
                counter_to_trigger_rs_curve=0;
                if order==2
                    connect_path=GenerateConnectPath(child_node_crab);                    
                elseif order==1
                    connect_path=GenerateConnectPatho1(child_node_crab,end_config);
                end
                [flag,connect_path_seg]=IsTrajValid(connect_path);  
                if(flag)
                    is_ha_search_complete=1;
...
%         end
    ...
    if(is_ha_search_completed_by_rs)
        path_segx=flip(connect_path_seg.x);
        path_segy=flip(connect_path_seg.y);
        path_segtheta=flip(connect_path_seg.theta);
        
        x=[x,path_segx(1,2:end-1)];
        y=[y,path_segy(1,2:end-1)];
        theta=[theta,path_segtheta(1,2:end-1)];
        pattern=[pattern,connect_path_seg.pattern(1,2:end-1)];
    end
...
function [expansion_pattern,expansion_crab,expansion_spin]=SpecifySample()
    ...
    for ii=1:params_.hybrid_astar.num_sampled_phy_in_crab
        expansion_crab=[expansion_crab;[-1,crab_list(ii)]];
    end
    xf=params_.task.xf;
    yf=params_.task.yf;
...
function Traj=GenerateConnectPath(child_node)
    global params_
    task_point=params_.task.task_point;
    Traj=[];
%     dubinsConnObj=robotics.DubinsConnection('MinTurningRadius',params_.vehicle.turning_radius_min/2);
    ...
    reedsConnObj.ReverseCost=1.0+params_.hybrid_astar.penalty_for_backward;
    for i=1:size(task_point,1)
%         [pathSegObj,~]=connect(dubinsConnObj,[task_point(i,1),task_point(i,2),task_point(i,3)],[child_node.x,child_node.y,child_node.theta]);
        [pathSegObj,~]=connect(reedsConnObj,[task_point(i,1),task_point(i,2),task_point(i,3)],[child_node.x,child_node.y,child_node.theta]);
        poses=interpolate(pathSegObj{1},[0:params_.hybrid_astar.resolution_dx:pathSegObj{1}.Length]);
        traj.x=poses(:,1)';
        traj.y=poses(:,2)';
        traj.theta=poses(:,3)';
...
%         hold on
    ...
    for i=1:size(task_point,1)
        ...
        posex=child_node.x;
        posey=child_node.y;
        posetheta=task_point(i,3);
        for k=1:point_num
           posex=[posex,task_point(i,1)+(k+1)*(child_node.x-task_point(i,1))/(point_num+1)];
           posey=[posey,task_point(i,2)+(k+1)*(child_node.y-task_point(i,2))/(point_num+1)];
           posetheta=[posetheta,task_point(i,3)];
        end
        traj.x=posex;
        traj.y=posey;
...
function Traj=GenerateConnectPatho1(child_node,end_config)
    global params_
    Traj=[];
%     dubinsConnObj=robotics.DubinsConnection('MinTurningRadius',params_.vehicle.turning_radius_min/2);
%     [pathSegObj,~]=connect(dubinsConnObj,[end_config(1,1),end_config(1,2),end_config(1,3)],[child_node.x,child_node.y,child_node.theta]);
    
    reedsConnObj=robotics.ReedsSheppConnection('MinTurningRadius',params_.vehicle.turning_radius_min/2);
    reedsConnObj.ReverseCost=1.0+params_.hybrid_astar.penalty_for_backward;
    [pathSegObj,~]=connect(reedsConnObj,[end_config(1,1),end_config(1,2),end_config(1,3)],[child_node.x,child_node.y,child_node.theta]); 
    ...
    for k=1:point_num
       posex=[posex,end_config(1,1)+(k+1)*(child_node.x-end_config(1,1))/(point_num+1)];
       posey=[posey,end_config(1,2)+(k+1)*(child_node.y-end_config(1,2))/(point_num+1)];
