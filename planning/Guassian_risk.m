...
u_total=0;
for i=1:size(x,2)
    u_total=u_total+compute_total_risk(x(1,i), y(1,i), distribution);
end
...
%     for i=1:(distribution(j).expire_time-distribution(j).start_time+1)
%         plot(distribution(j,1).state(i).mu(1),distribution(j,1).state(i).mu(2),'r*');
%         hold on;
%     end
% end
    for j=1:N
        for i = 1:(pedestrians(j).expire_time+1-pedestrians(j).start_time)
            mu_i = (pedestrians(j).state(i).mu)';        % [x_i; y_i]
            cov_i = pedestrians(j).state(i).Sigma;      % 2x2 协方差矩阵
