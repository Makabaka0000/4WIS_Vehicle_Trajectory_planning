...
minimize obj: 
tf +
w_penalty * (sum{i in {2..Nfe}}((x[i] - x[i-1] - hi * v[i] * cos(theta[i]))^2 + (y[i] - y[i-1] - hi * v[i] * sin(theta[i]))^2 + (v[i] - v[i-1] - hi * a[i])^2 + 
(theta[i] - theta[i-1] - hi * tan(phy[i]) * v[i] / L_wheelbase)^2 + (phy[i] - phy[i-1] - hi * w[i])^2 + (xf[i] - x[i] - Lfc * cos(theta[i]))^2 + 
(yf[i] - y[i] - Lfc * sin(theta[i]))^2 + (xr[i]  - x[i] - Lrc * cos(theta[i]))^2 + (yr[i] - y[i] - Lrc * sin(theta[i]))^2) + (sin(theta[Nfe]) - sin(SixBoundaryValues[6]))^2 + 
(cos(theta[Nfe]) - cos(SixBoundaryValues[6]))^2);
...
s.t. EQ_cd {i in {1..Nfe-1}}:
    ((a[i+1] + 2 * amax) * hi^2 + 6 * v[i] * hi)^2 = 36 * ((x[i+1] - x[i])^2 + (y[i+1] - y[i])^2);
