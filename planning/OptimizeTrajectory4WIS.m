function OptimizeTrajectory4WIS()
    load('paradebug.mat')
    global params_
    ...
    limo_a=[];
    limo_w=[];
    limo_terminal_time=0; 
    
    for i=1:size(params_.ha_result.Index,1)
        start_index=params_.ha_result.Index(i,1);
        end_index=params_.ha_result.Index(i,2);
        pattern=params_.ha_result.Index(i,3);
        terminal_time=params_.ha_result.Index(i,4);
        
        x=params_.ha_result.x(1,start_index:end_index);
        y=params_.ha_result.y(1,start_index:end_index);
        theta=params_.ha_result.theta(1,start_index:end_index);
        v=params_.ha_result.v(1,start_index:end_index);
        a=params_.ha_result.a(1,start_index:end_index);
        phy=params_.ha_result.phy(1,start_index:end_index);
        w=params_.ha_result.w(1,start_index:end_index);        
        
        Form4WISInitialGuess(x,y,theta,v,a,phy,w,terminal_time,pattern);    
        Write4WISBoundartValues(x,y,theta);
        Write4WISBasicParameterFile(size(x,2));
...
%             end
            ...
            if(infeasibility<params_.opti.acceptance_tolerance)
                limo_x=[limo_x,x];
                limo_y=[limo_y,y];
                limo_theta=[limo_theta,theta];
                limo_phy=[limo_phy,phy];
                limo_v=[limo_v,v];
                limo_a=[limo_a,a];
                limo_w=[limo_w,w];
                limo_terminal_time=limo_terminal_time+terminal_time; 
                break;
            end
        end
    end
    ...
    params_.limo.x=limo_x;
    params_.limo.y=limo_y;
    params_.limo.theta=limo_theta;
    params_.limo.phy=limo_phy;
    params_.limo.v=limo_v;
    params_.limo.a=limo_a;
    params_.limo.w=limo_w;
    params_.limo.terminal_time=limo_terminal_time;
