function[x,y,theta,pattern]=SearchBackwardHybridAstar(start_config,end_config)
    ...
    while((~isempty(openlist_))&&(ha_iter<=params_.hybrid_astar.max_search_iter)&&(~is_ha_search_complete))
        ...
        for ii=1:(2*params_.hybrid_astar.num_sampled_phy_in_expansion)
            ...
            if(child_node.via_v>0)
                child_g=child_g+1000*params_.hybrid_astar.penalty_for_backward;
            end
            ...
            if(counter_to_trigger_rs_curve>params_.hybrid_astar.threshold_to_trigger_rs)
                counter_to_trigger_rs_curve=0;
%                 rs_path_seg=GenerateDuBinsPath(child_node,end_config);
%                 if(Is3DTrajValid(rs_path_seg.x,rs_path_seg.y,rs_path_seg.theta))
%                 [flag,rs_path]=IsRsTrajValid(rs_path_seg);
%                 if(flag)
       ...
       %拓展斜向运动的节点
        for ii = 1 : size(expansion_crab,1)
            ...
            if(child_node_crab.via_v>0)
                child_crab_g=child_crab_g+1000*params_.hybrid_astar.penalty_for_backward;
            end            
            ...
            if(counter_to_trigger_rs_curve>params_.hybrid_astar.threshold_to_trigger_rs)
                counter_to_trigger_rs_curve=0;
%                 rs_path_seg=GenerateRsPath(child_node_crab,end_config);
...
function traj=GenerateDuBinsPath(child_node,end_config)
    global params_
    dubinsConnObj=robotics.DubinsConnection('MinTurningRadius',params_.vehicle.turning_radius_min);
%     [pathSegObj,~]=connect(dubinsConnObj,[point1_x, point1_y,point1_theta],[point2_x, point2_y,point2_theta]);
%     
%     reedsConnObj=robotics.ReedsSheppConnection('MinTurningRadius',params_.vehicle.turning_radius_min);
%     reedsConnObj.ReverseCost=1.0+params_.hybrid_astar.penalty_for_backward;
...
%         [pathSegObj,~]=connect(dubinsConnObj,[task_point(i,1),task_point(i,2),task_point(i,3)],[child_node.x,child_node.y,child_node.theta]);
        ...
        if i+params_.search_index>size(params_.search_point,1)-params_.task.task_point_num
            traj.index=size(params_.search_point,1)-params_.task.task_point_num;
        else
            traj.index=i+params_.search_index;
        end
...
%         yf=y+params_.vehicle.FWISf2p*sin(theta);
        ...
        is_collision_free=1;
        rs_seg=Traj(i,1);
        params_.search_index=Traj(i,1).index;
        break;
    end
end
function is_collision_free=IsDubinsTrajValid(Traj)
    global params_
    is_collision_free=0;
    x=Traj.x;
    y=Traj.y;
