function SearchTrajectoryViaFTHA()
    global params_
    [x_test,~,~]=SearchAStarPath([params_.task.x0,params_.task.y0,params_.task.theta0],[params_.task.xf,params_.task.yf,params_.task.thetaf]);
    if(isempty(x_test))
        params_.ha_result.x=[];
        params_.ha_result.y=[];
        params_.ha_result.theta=[];
        return;
    end
    ...
    if(~ready_flag)
        [x_modify,y_modify,theta_modify]=SearchAStarPath([xr(end),yr(end),theta(end)],[params_.task.xf,params_.task.yf,params_.task.thetaf]);
        x=[x,x_modify];
        y=[y,y_modify];
        theta=[theta,theta_modify];
    end
    ...
    x0=[15,-7];bia_x0=[0.5,0.4];
    y0=[-7,-5];bia_y0=[0.5,0.3];
...
function CreateCostmaps()
    ...
    for i=1:size(distribution,1)
        for j=1:size(distribution(i,1).x,2)
            obs.x=distribution(i,1).x(:,j).mu;
            obs.y=distribution(i,1).y(:,j).mu;
            addobs=[addobs,obs];
        end
    end
    ...
    for ii = 1 : size(addobs, 2)
        ...
        for jj = Nmin_x : Nmax_x
            for kk = Nmin_y : Nmax_y
                ...
                vx = [linspace(x0, x1, 5), linspace(x1, x1, 5), linspace(x1, x0, 5), linspace(x0, x0, 5)];
                vy = [linspace(y1, y1, 5), linspace(y1, y0, 5), linspace(y0, y0, 5), linspace(y0, y1, 5)];
                if (any(inpolygon(vx, vy, addobs(ii).x,addobs(ii).y)))
                    costmap(jj, kk) = 1;
                end
            end
        end
