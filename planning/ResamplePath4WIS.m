function[x1,y1,theta1,v,a,phy,w,terminal_time]=ResamplePath4WIS(Index_seg,x_all,y_all,theta_all)
    global params_
    %         x(1,Index_seg(1,1):Index_seg(1,2)),y(1,Index_seg(1,1):Index_seg(1,2)),theta(1,Index_seg(1,1):Index_seg(1,2))
    x=x_all(1,Index_seg(1,1):Index_seg(1,2));y=y_all(1,Index_seg(1,1):Index_seg(1,2));theta=theta_all(1,Index_seg(1,1):Index_seg(1,2));pattern=Index_seg(1,3);
    nfe=round((Index_seg(1,2)-Index_seg(1,1)+1)*params_.opti.nfe/size(x_all,2));
    ...
    if(size(x,2)==1)
        x1=ones(1,nfe).*x;
        y1=ones(1,nfe).*y;
        theta1=ones(1,nfe).*theta;
        v=zeros(1,nfe);
        a=zeros(1,nfe);
        phy=zeros(1,nfe);
        w=zeros(1,nfe);
        terminal_time=zeros(1,nfe);
        return;
    end
    ...
    for ii=2:(Nfe-1)
        if pattern==0
            phy(ii)=atan((theta1(ii+1)-theta1(ii))*params_.vehicle.lw/(2*dt*v(ii)));
        elseif pattern==2
            phy(ii)=asin((y1(ii)-y1(ii-1))/(dt*v(ii)))-theta(1);
        end
        if(phy(ii)>phy_max)
            phy(ii)=phy_max;
    ...
    index=round(linspace(1,length(x1),nfe));
    x1=x1(index);
    y1=y1(index);
    theta1=theta1(index);
    v=v(index);
